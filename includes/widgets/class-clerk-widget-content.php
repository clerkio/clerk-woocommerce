...
class Clerk_Widget_Content extends WP_Widget {
    ...
    public function form( $instance ) {
        ...
        <p>
            ...
            <select name="<?php echo $this->get_field_name('content'); ?>" id="<?php echo $this->get_field_id('content'); ?>" onchange="clerkGetContent(this)">
                <option value=""><?php _e('Select Content', 'clerk'); ?></option>
                <?php foreach ( $contents->contents as $content ) : ?>
                    <?php if ( $content->type !== 'html' ) continue; ?>
                    <option value="<?php echo esc_attr($content->id); ?>"<?php if($instance['content'] === $content->id) : ?>selected<?php endif; ?>><?php echo $content->name; ?></option>
                <?php endforeach; ?>
            </select>
        </p>
